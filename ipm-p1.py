...
class Model():
	...
	def __init__(self):
		...
		self.store_day = []
		self.store_day.append([ today.strftime("%x"), "20", "categoria1", "Calentamiento previo"])
		self.store_day.append([ today.strftime("%x"), "12", "categoria2", "Pilates"])
		self.store_day.append([ today.strftime("%x"), "14", "categoria2", "Yoga"])
		self.store_day.append([ today.strftime("%x"), "34", "categoria4", "Spinning"])

		self.store_month = []
		self.store_month.append([ datetime.date(2018,10,3).strftime("%x"), "20", "categoria1", "Calentamiento previo"])
		self.store_month.append([ datetime.date(2018,10,10).strftime("%x"), "12", "categoria2", "Pilates"])
		self.store_month.append([ today.strftime("%x"), "14", "categoria2", "Yoga"])
		self.store_month.append([ datetime.date(2018,10,14).strftime("%x"), "34", "categoria4", "Spinning"])
	...
	#Método para buscar entradas en un día
	def find(self, date):
		if date != None:
...
class ViewController:
    ...
    def on_find_day_clicked(self, w):
    	...
    	r = self.model.find(date)
    	self.view.find_day(r)

    def on_find_entries_clicked(self, w):
    	month = self.view.get_month()
    	r = self.model.find_month(month)
    	self.view.entries_by_month(r)

    def on_summary_month_clicked(self, w):
    	month = self.view.get_month()
    	r = self.model.find_month(month)
    	self.view.summary_of_month(r)    
...
class View:
    def __init__(self):
        ...
        self.summary = Gtk.Button(label="Resumen del mes")
        self.filter_day = Gtk.Button(label="Filtrar dia")
        self.entry = Gtk.Button(label="Entradas introducidas")
    ...
    def get_date_filter(self):
        ...
        if response == Gtk.ResponseType.OK:
            result =  str(dDate) + "/" + str(mDate) + "/" + str(aDate)            
        else:
            result = None
        dialog.destroy()
        return result
    ...
    def get_month(self):
        	...
        	year = Gtk.Entry()
        	grid.attach(Gtk.Label("Mes: "), 0, 0, 1, 1)
        	grid.attach(month, 1, 0, 1, 1)
        	grid.attach(Gtk.Label("Año"), 0, 1, 1, 1)
        	grid.attach(year, 1, 1, 1, 1)
        	box.pack_start(grid, True, True, 0)
        	box.show_all()
        	response = dialog.run()      
        	if response == Gtk.ResponseType.OK:
        		result = "01/" + month.get_active_text() + "/" + year.get_text().strip()        
        	else:
        		result = None
        	dialog.destroy()
        	return result
